...
#   # 不设 KS_PWD 时会随机生成并写入 signature/.keystore.pwd（权限 600，已被 .gitignore 忽略）
set -uo pipefail
...
[[ -f "$SIGN_TOOL" ]] || { echo "[csr] 找不到 hap-sign-tool.jar：$SIGN_TOOL" >&2; exit 2; }
command -v java >/dev/null 2>&1 || { echo "[csr] 缺少 java（见 ENVIRONMENT.md §1）" >&2; exit 2; }

SIG_DIR="$ROOT_DIR/signature"
ALIAS="${PD_KEY_ALIAS:-photodelete}"
KEYSTORE="$SIG_DIR/photodelete.p12"
PWD_FILE="$SIG_DIR/.keystore.pwd"
CSR="$SIG_DIR/photodelete.csr"
...
if [[ -z "${KS_PWD:-}" ]]; then
  if [[ -f "$PWD_FILE" ]]; then
    KS_PWD="$(cat "$PWD_FILE")"
    echo "[csr] 复用已有口令：$PWD_FILE"
  else
    KS_PWD="$(head -c 24 /dev/urandom | base64 | tr -d '/+=' | head -c 20)"
    printf '%s' "$KS_PWD" > "$PWD_FILE"
    chmod 600 "$PWD_FILE"
    echo "[csr] 已随机生成密钥库口令并写入 $PWD_FILE（权限 600，勿入库）"
  fi
fi

if [[ -f "$KEYSTORE" ]]; then
  echo "[csr] 密钥库已存在，跳过生成：$KEYSTORE"
else
  echo "[csr] 生成密钥对（ECC NIST-P-256）..."
